...
const { acquireSource, downloadArchive, downloadBranch } = await import('../download.js');
...
`, { mode: 0o755 });
  ...
  it('dispatches a github-release commit source without inventing a v-prefixed tag', () => {
    const fixtureDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zylos-acquire-commit-fixture-'));
    const archiveRoot = path.join(fixtureDir, 'repo-commit');
    const archivePath = path.join(fixtureDir, 'archive.tar.gz');
    const fakeBin = fs.mkdtempSync(path.join(os.tmpdir(), 'zylos-acquire-commit-bin-'));
    const callsPath = path.join(fakeBin, 'curl.calls');
    tmpDirs.push(fixtureDir, fakeBin);
...
`, { mode: 0o755 });
    ...
    assert.match(calls, new RegExp(`https://github\\.com/org/repo/archive/${sha}\\.tar\\.gz`));
    assert.doesNotMatch(calls, new RegExp(`/archive/refs/tags/v${sha}\\.tar\\.gz`));
  });

  it('fails closed before creating a destination for an invalid github-release commit source', () => {
    const fake = installFailingCurl({ status: '404' });
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'zylos-acquire-invalid-source-'));
    ...
    assert.equal(result.success, false);
    assert.match(result.error, /full 40-hex SHA/);
