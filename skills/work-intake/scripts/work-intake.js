...
const YUERAN_ASSIGNMENT = /(?:交给|让|请|麻烦|安排)\s*@?玥然|@?玥然\s*(?:来|负责|处理|完成|跟进|整理|帮)|(?:执行人|负责人|处理人|承办人)\s*(?:是|为|[：:])?\s*@?玥然/u;
...
function defaultAssignee(options) {
  if (options === undefined) return null;
  if (!options || typeof options !== 'object' || Array.isArray(options)) {
    throw new TypeError('WorkIntake classifier options must be an object');
  }
  const unknown = Object.keys(options).find(key => key !== 'defaultAssigneeId');
  if (unknown) throw new TypeError(`WorkIntake classifier options contain unsupported field: ${unknown}`);
  if (options.defaultAssigneeId === undefined || options.defaultAssigneeId === null) return null;
  ...
  return options.defaultAssigneeId.trim();
}
...
function taskDirectiveText(text) {
  return text
    .replace(/^(?:\s*(?:【[^】\r\n]{1,80}】|\[[^\]\r\n]{1,80}\])\s*)+/u, '')
    .replace(/^@?玥然\s*[，,：:、]\s*/u, '')
    .trim();
...
export function hasExplicitYueranAssignment(text) {
  return typeof text === 'string' && YUERAN_ASSIGNMENT.test(taskDirectiveText(text));
}

function personResolution(envelope, text = envelope.text) {
  if (YUERAN_ASSIGNMENT.test(text)) {
  ...
  const match = text.match(HUMAN_ASSIGNMENT);
...
function buildTaskDraft(envelope, {
  ...
  const title = taskTitle(directiveText, dueText);
  const details = [];
...
export function classify(input, options) {
  ...
  const configuredDefaultAssignee = defaultAssignee(options);
  const directiveText = taskDirectiveText(envelope.text);
  const explicitTask = EXPLICIT_TASK_PREFIX.test(directiveText);
  const yueranAssigned = hasExplicitYueranAssignment(directiveText);
  const people = personResolution(envelope, directiveText);
  const assigneeId = yueranAssigned
    ? 'agent:yueran'
    : (people.assigneeId ?? (people.explicit ? null : configuredDefaultAssignee));
  ...
  if (HIGH_RISK_ACTION.test(directiveText)) {
    return result(
      ...
      buildTaskDraft(envelope, { riskLevel: 'high', assigneeId, directiveText }),
    );
  }
  ...
  if (people.ambiguous) {
    return result(
      ...
      'confirm',
      'PERSON_AMBIGUOUS',
      buildTaskDraft(envelope, { assigneeId: null, directiveText }),
    );
  }

  if (VAGUE_TIME.test(directiveText) && (explicitTask || yueranAssigned || DURABLE_ACTION.test(directiveText))) {
    return result(
      envelope,
      'confirm',
      'TIME_AMBIGUOUS',
      buildTaskDraft(envelope, { assigneeId, directiveText }),
    );
  }
  ...
  if (yueranAssigned && durableAction && !ONE_SHOT_REQUEST.test(directiveText)) {
    return result(
      envelope,
      'create_task',
      'EXPLICIT_ASSIGNMENT',
      buildTaskDraft(envelope, { assigneeId: 'agent:yueran', directiveText }),
    );
  }
  ...
  if (people.explicit && people.assigneeId && durableAction && !ONE_SHOT_REQUEST.test(directiveText)) {
    return result(
      ...
      buildTaskDraft(envelope, { assigneeId: people.assigneeId, directiveText }),
    );
  }
