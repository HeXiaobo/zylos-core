...
export function prepare(options) {
  ...
  for (const name of ['core', 'feishu', 'hxa']) {
    ...
    const selected = selectTag(git('tag', '--list').split('\n'), options[`--${name}`] || 'latest', options['--channel'] || 'fork');
    const sha = git('rev-parse', `refs/tags/${selected.tag}^{commit}`);
  ...
  write('bundle.json', { ...previous, releaseId, candidate });
  ...
  return { status: 'PREPARED', directory: output, releaseId, candidate, deploymentAllowed: false, runtimeMutation: false, next: 'Read WORKFLOW.md; verify host, rollback baseline, compatibility and evidence before READY.' };
}
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  try {
    const args = process.argv.slice(2), options = {};
    if (args.includes('--help')) {
      console.log('node tools/upgrade/prepare.mjs --out NEW_ABSOLUTE_DIR --authorization-ref MESSAGE_ID [--core latest|VERSION] [--feishu latest|VERSION] [--hxa latest|VERSION] [--channel fork|stable]');
    } else {
      for (let i = 0; i < args.length; i += 2) {
        if (!['--out', '--authorization-ref', '--core', '--feishu', '--hxa', '--channel'].includes(args[i]) || !args[i + 1] || args[i + 1].startsWith('--') || options[args[i]]) throw new Error('Invalid arguments; use --help');
